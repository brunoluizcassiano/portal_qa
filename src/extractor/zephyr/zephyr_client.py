...
def run_extracao_zephyr_diaria(
   ...
   base = zephyr_cfg.get("base_url") or os.getenv("ZEPHYR_BASE_URL")
   token = zephyr_cfg.get("api_token") or os.getenv("ZEPHYR_API_TOKEN")
   if not (base and token):
       raise ValueError("[zephyr] configure ZEPHYR_BASE_URL e ZEPHYR_API_TOKEN (env) ou no secrets.toml (sem segredos).")
   zc = ZephyrClient(base_url=base, api_token=token)
   ...
   # versões "latest"
   df_tcs.to_csv(BASE_DIR / "zephyr_testcases_latest.csv", index=False)
   df_exec.to_csv(BASE_DIR / "zephyr_testexecutions_latest.csv", index=False)
   df_cyc.to_csv(BASE_DIR / "zephyr_testcycles_latest.csv", index=False)
   df_stat.to_csv(BASE_DIR / "zephyr_statuses_latest.csv", index=False)
   ...
   return {
       ...
       "latest": [
           str(BASE_DIR / "zephyr_testcases_latest.csv"),
           str(BASE_DIR / "zephyr_testexecutions_latest.csv"),
           str(BASE_DIR / "zephyr_testcycles_latest.csv"),
           str(BASE_DIR / "zephyr_statuses_latest.csv"),
       ],
       "concurrency": {
           "max_workers": MAX_WORKERS,
           "page_size": PAGE_SIZE,
