...
def _monthly_series_negative_cumulative(df_zc_f: pd.DataFrame) -> pd.DataFrame:
    """
    Série mensal de % Negative Test usando a MESMA lógica do card.

    Para cada mês M (visão cumulativa):
      - considera todos os test cases criados até o fim de M (df_zc_f filtrado)
      - aplica kpi_negative_now(df_cum) para calcular o percentual.
    ...
    for m in months:
        ...
        if df_cum.empty:
            ...
            # usa exatamente a mesma lógica do card
            value = float(kpi_negative_now(df_cum))
