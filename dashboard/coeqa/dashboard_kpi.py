...
def pagina_dashboard_kpi():
   ...
   st.markdown("""
    <style>
    /* grid opcional se quiser alinhar as 7 colunas com gap */
    ...
    background: rgba(186,85,211,.16);
    border-color: transparent;
    }

    /* Estados por KPI (cores bem fáceis de trocar) */
    .kpi-card.kpi-below  { border-color: #d9534f; background: rgba(217,83,79,.16); }
    .kpi-card.kpi-near   { border-color: #f0ad4e; background: rgba(240,173,78,.16); }
    .kpi-card.kpi-ok     { border-color: #5cb85c; background: rgba(92,184,92,.16); }
   ...
   # helper: set de issueIds do projeto selecionado
   ...
   def kpi_auto_runs_now():
       tmp = executions_filtered_by_project()
   ...
   if "kpi_selected" not in st.session_state:
       ...
       # Sincroniza seleção via query string (?kpi=coverage)
       params = st.experimental_get_query_params()
       if "kpi" in params and params["kpi"]:
        st.session_state["kpi_selected"] = params["kpi"][0]

   # ---------- cards-botão ----------
   def kpi_card(col, key, label, value, extra_cls=""):
    selected = (st.session_state["kpi_selected"] == key)
    classes = "kpi-card " + (extra_cls or "")
    if selected:
        classes += " kpi-selected"

    # quebra de linha do valor (ex.: "Target ...\nΔ +1.2pp")
    val_html = (value or "").replace("\n", "<br>")

    # usar link com query param para selecionar e recarregar a página
    href = f"?kpi={key}"
    with col:
        st.markdown(
            f'''<a class="{classes}" href="{href}">
                    <span class="kpi-title">{label}</span>
                    <span class="kpi-value">{val_html}</span>
                </a>''',
            unsafe_allow_html=True
        )

   cols = st.columns(7)
   kpi_card(cols[0], "coverage",  KPI_DEFS["coverage"]["title"],  KPI_DEFS["coverage"]["value"],  KPI_CLASS["coverage"])
   kpi_card(cols[1], "test_avg",  KPI_DEFS["test_avg"]["title"],  KPI_DEFS["test_avg"]["value"],  KPI_CLASS["test_avg"])
   kpi_card(cols[2], "auto_reg",  KPI_DEFS["auto_reg"]["title"],  KPI_DEFS["auto_reg"]["value"],  KPI_CLASS["auto_reg"])
   kpi_card(cols[3], "auto_runs", KPI_DEFS["auto_runs"]["title"], KPI_DEFS["auto_runs"]["value"], KPI_CLASS["auto_runs"])
   kpi_card(cols[4], "test_reg",  KPI_DEFS["test_reg"]["title"],  KPI_DEFS["test_reg"]["value"],  KPI_CLASS["test_reg"])
   kpi_card(cols[5], "negative",  KPI_DEFS["negative"]["title"],  KPI_DEFS["negative"]["value"],  KPI_CLASS["negative"])
   kpi_card(cols[6], "bug_days",  KPI_DEFS["bug_days"]["title"],  KPI_DEFS["bug_days"]["value"],  KPI_CLASS["bug_days"])
   st.markdown("---")
...
#    st.markdown("---")
   ...
   df_func_all = df_func.copy()
   df_epic_all = df_epic.copy()
   df_story_all = df_story.copy()
